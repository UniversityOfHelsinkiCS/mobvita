...
const ExerciseCloze = ({ word, handleChange, handleClick }) => {
  ...
  return (
    <Input
      ...
      transparent
      style={{ minWidth: `${Math.floor(word.base ? word.base.length : word.bases.length)}em`, width: `${Math.floor(word.surface.length) + 10}em`, height: '30px', backgroundColor: color }}
    />
  )
}
